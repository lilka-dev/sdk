...
static void *elf_malloc(uint32_t n, bool exec) {
    (void)exec;
    /* On ESP32-S3 with PSRAM, we allocate from SPIRAM for both text and data.
     * Text will be remapped to instruction bus via address offset. */
...
static int elf_arch_relocate(lilka_elf_t *elf, const lilka_elf32_rela_t *rela,
                             const lilka_elf32_sym_t *sym, uint32_t addr) {
...
static int elf_load_sections(lilka_elf_t *elf, const uint8_t *pbuf) {
    ...
    const lilka_elf32_shdr_t *shdr =
        (const lilka_elf32_shdr_t *)(pbuf + ehdr->shoff);
    const char *shstrtab =
        (const char *)pbuf + shdr[ehdr->shstrndx].offset;

    /* Find relevant sections */
    for (uint32_t i = 0; i < ehdr->shnum; i++) {
        const char *name = shstrtab + shdr[i].name;

        if (shdr[i].type == LILKA_SHT_PROGBITS &&
            (shdr[i].flags & LILKA_SHF_ALLOC)) {

            if ((shdr[i].flags & LILKA_SHF_EXECINSTR) &&
                strcmp(".text", name) == 0) {
                elf->sec[LILKA_ELF_SEC_TEXT].v_addr = shdr[i].addr;
                elf->sec[LILKA_ELF_SEC_TEXT].size =
                    LILKA_ELF_ALIGN(shdr[i].size, 4);
                elf->sec[LILKA_ELF_SEC_TEXT].offset = shdr[i].offset;
            } else if ((shdr[i].flags & LILKA_SHF_WRITE) &&
                       strcmp(".data", name) == 0) {
                ...
                elf->sec[LILKA_ELF_SEC_DRLRO].offset = shdr[i].offset;
                   ...
                   (shdr[i].flags & (LILKA_SHF_ALLOC | LILKA_SHF_WRITE)) ==
                       (LILKA_SHF_ALLOC | LILKA_SHF_WRITE) &&
                   strcmp(".bss", name) == 0) {
    ...
    /* Allocate text section in PSRAM */
    elf->ptext =
        (unsigned char *)elf_malloc(elf->sec[LILKA_ELF_SEC_TEXT].size, true);
    if (!elf->ptext) {
        lilka::serial.log("dynloader: failed to alloc %u bytes for .text",
                          elf->sec[LILKA_ELF_SEC_TEXT].size);
        return -ENOMEM;
    }

    /* Allocate data sections */
    uint32_t data_size = elf->sec[LILKA_ELF_SEC_DATA].size +
                         elf->sec[LILKA_ELF_SEC_RODATA].size +
                         elf->sec[LILKA_ELF_SEC_BSS].size +
                         elf->sec[LILKA_ELF_SEC_DRLRO].size;
    if (data_size) {
        elf->pdata = (unsigned char *)elf_malloc(data_size, false);
        if (!elf->pdata) {
            lilka::serial.log("dynloader: failed to alloc %u bytes for data",
                              data_size);
            elf_free(elf->ptext);
            elf->ptext = nullptr;
            return -ENOMEM;
    ...
    /* Copy .text into executable memory */
    ...
    uintptr_t entry_addr = ehdr->entry +
                           elf->sec[LILKA_ELF_SEC_TEXT].addr -
...
int lilka_elf_relocate(lilka_elf_t *elf, const uint8_t *pbuf, size_t size) {
    ...
    for (uint32_t i = 0; i < ehdr->shnum; i++) {
        if (shdr[i].type == LILKA_SHT_RELA) {
            uint32_t nr_reloc =
                shdr[i].size / sizeof(lilka_elf32_rela_t);
            const lilka_elf32_rela_t *rela =
                (const lilka_elf32_rela_t *)(pbuf + shdr[i].offset);
            const lilka_elf32_sym_t *symtab =
                (const lilka_elf32_sym_t *)(pbuf + shdr[shdr[i].link].offset);
            const char *strtab =
                (const char *)(pbuf + shdr[shdr[shdr[i].link].link].offset);

            for (uint32_t j = 0; j < nr_reloc; j++) {
                lilka_elf32_rela_t rela_buf;
                memcpy(&rela_buf, &rela[j], sizeof(lilka_elf32_rela_t));

                const lilka_elf32_sym_t *sym =
                    &symtab[LILKA_ELF_R_SYM(rela_buf.info)];
                int type = LILKA_ELF_R_TYPE(rela_buf.info);
                uintptr_t addr = 0;

                if (type == LILKA_STT_COMMON || type == LILKA_STT_OBJECT ||
                    type == LILKA_STT_SECTION) {
                    ...
                    if (sym_name[0]) {
                        ...
                        if (!addr && sym->shndx != LILKA_SHN_UNDEF) {
                            addr = (uintptr_t)(elf->sec[LILKA_ELF_SEC_DATA].addr +
                                               sym->value -
                    ...
                    /* Check local text */
                    if (!addr && sym->shndx != LILKA_SHN_UNDEF) {
                        addr = (uintptr_t)(elf->sec[LILKA_ELF_SEC_TEXT].addr +
                                           sym->value -
                                           elf->sec[LILKA_ELF_SEC_TEXT].v_addr);
                    }
                    if (!addr) {
        ...
        /* Parse .dynsym for exported symbols */
            ...
            const lilka_elf32_sym_t *dsymtab =
                (const lilka_elf32_sym_t *)(pbuf + shdr[i].offset);
            const char *dstrtab =
                (const char *)(pbuf + shdr[shdr[i].link].offset);
            ...
            if (count) {
                elf->symtab = (lilka_dynsym_t *)elf_malloc(
                    count * sizeof(lilka_dynsym_t), false);
                ...
                for (uint32_t j = 0; j < nsyms && idx < count; j++) {
                    if (LILKA_ELF_ST_BIND(dsymtab[j].info) == LILKA_STB_GLOBAL &&
                        ...
                        elf->symtab[idx].addr = (void *)(
                            elf->ptext + dsymtab[j].value -
                            elf->sec[LILKA_ELF_SEC_TEXT].v_addr);
                        /* Copy name */
                        size_t len =
                            strlen(dstrtab + dsymtab[j].name) + 1;
...
static void _register_libc_symbols() {
